...
scheduler.SubmitArguments='-q compute -l mem=72gb -l walltime=460800';
...
for sub = [5,11,17,6,18,19]
    ...
    for rois = 1:numel(roi_list)
        ...
        data3Dtrain = single(Dvis.fttimelock.trial);
        cond_values_train = Dvis.fttimelock.trialinfo;
        ...
        data3Dtest = single(Dattn.fttimelock.trial);
        ...
        images=cell(1,2);
        for i=1:2
            images{i} = find(cond_values_train==choosetargets(i));
        end
        ...
        for smooth = 1:numel(smoothing)
            ...
            for winsize = winsizes
                ...
                output_dir = 'crossgen_ROIs_vis-3itemT';
                try cd(output_dir)
