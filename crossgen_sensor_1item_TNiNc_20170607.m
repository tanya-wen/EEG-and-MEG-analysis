...
for sub = subs
    ...
    before_T_Ni = intersect(before_T_ind,find(TNiNc==2));
    Ni_ind = find(TNiNc==2);
    before_Ni_ind = Ni_ind-1;
    before_Ni_T = intersect(before_Ni_ind,find(TNiNc==1));
    D = badtrials(D, before_T_Ni+1, 1);
    D = badtrials(D, before_Ni_T+1, 1);
