...
addpath('/imaging/tw05/Preparatory_Attention_Study/software/libsvm-mat-2.87-1');
% control variables
...
for sub = subs
    ...
    roi_list = {'MD_ESV.nii','MD_IPS.nii','MD_AI.nii','MD_postMFG.nii','MD_ACCpreSMA.nii',...
        'MD_FEF.nii','MD_antMFG.nii','MD_midMFG.nii','Auditory_Te3.nii','Visual_hOc1.nii','LPFC.nii'};
    ...
    for rois = 1:numel(roi_list)
        ...
        D.inv{val}.source.fname = strcat(roinames{rois},'_vis');
        D.inv{val}.source.type  = 'trials';
        spm_eeg_inv_extract_tw(D); % added XYZ = unique(XYZ,'rows') to stop duplicate vertices
        
        Dvis = spm_eeg_load(strcat(roinames{rois},'_vis'));
        data3Dtest = single(Dvis.fttimelock.trial);
        cond_values_test = Dvis.fttimelock.trialinfo;
        ...
        data3Dtrain = single(Dattn.fttimelock.trial);
        cond_values_train = Dattn.fttimelock.trialinfo;
        ...
        images=cell(1,3);
        for i=1:3
            images{i} = find(cond_values_test==i);
        end
        ...
        cues=cell(1,2);
        for i=1:2
            cues{i} = find(cond_values_train==i);
        end
        ...
        for smooth = 1:numel(smoothing)
            ...
            for winsize = winsizes
                ...
                parfor islide = 1:ntrains
                    itrain = data3Dtrain(:,:,trainwins(islide,1):trainwins(islide,2));
                    ...
                    for jslide = 1:ntests
                        itest = data3Dtest(:,:,testwins(jslide,1):testwins(jslide,2));
                ...
                output_dir = 'crossgen_ROIs_prep-vis';
                try cd(output_dir)
