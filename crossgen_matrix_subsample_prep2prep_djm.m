            ...
            for winsize = winsizes
                ...
                for subsample = 1:nsubsample
                    ...
                    for fold = 1: nfolds
                        ...
                        c1 = datasample(find(train_cond_values==1),round(min_trials_mvpa(sub)/2),'Replace',false);
                        c2 = datasample(find(train_cond_values==2),round(min_trials_mvpa(sub)/2),'Replace',false);
                ...
                saveas(gcf,sprintf('CrossGenMatrix2_%03dms%s_%s_sub%s.png',winsize*4,smoothing{smooth},roinames{rois},num2str(sub)));
                save(sprintf('CrossGenMatrix2_%03dms%s_%s_sub%s.mat',winsize*4,smoothing{smooth},roinames{rois},num2str(sub)));
                close;
                cd(workingdir)
                cd(swd)
