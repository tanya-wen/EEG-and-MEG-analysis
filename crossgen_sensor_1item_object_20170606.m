%% Cross-generalization of which picture the participant is viewing in 1-item displays. Done separately within each ROI
% using only the two target items (so won't be confounded with behavioral category)
...
scheduler.SubmitArguments='-q compute -l mem=96gb -l walltime=460800';
...
for sub = [5,11,17,6,18,19]
    ...
    cd(swd)
        ...
        images=cell(1,2);
        for i=1:2
            images{i} = D.indtrial(sprintf('image%d',choosetargets(i)), 'GOOD');
%             images{i} = find(cond_values==choosetargets(i));
        end
        ...
        for smooth = 1:numel(smoothing)
            ...
            for winsize = winsizes
                ...
                for fold = 1: nfolds
                    testind=[images{1}(fold:nfolds:end) images{2}(fold:nfolds:end)];
                    labels_test=cond_values(testind)';
                    
                    trainind=setdiff([images{:}],testind);
                ...
                output_dir = 'crossgen_sensor_1item_object';
                try cd(output_dir)
